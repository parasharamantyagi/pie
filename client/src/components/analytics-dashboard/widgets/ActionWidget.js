...
class ActionWidget extends Component {
  ...
  render() {
    ...
    orgProjectActionStatus &&
      orgProjectActionStatus.forEach((p) => {
        ...
        if (!projectActionStatusData[status]) {
          ...
          projectActionStatusData[status].total =
            1 + projectActionStatusData[status].total;
          if (
            projectActionStatusData[status].projectsAdded.indexOf(p.title) ===
            -1
          ) {
            projectActionStatusData[status].projectsAdded.push(p.title);
            projectActionStatusData[status].projects.push({
    ...
    const data = [];
    // console.log("1111111111111", projectActionStatusData);

    Object.keys(projectActionStatusData.active.projects).forEach((p) => {
    //   console.log("iiiiiiii", p);
      data.push({
        name: dataFormat.New.name,
        y: projectActionStatusData.active.total,
        color: dataFormat.New.color,
        drilldown: p,
      });
      drilldownSeries.push({
        name: p,
        id: p,
        data: projectActionStatusData.active.projects,
      });
    });
