...
class OrganizationDocumentUpdate extends React.Component {
  constructor(props) {
    super(props);
...
console.log('idididididd-',id);
    if (id > 0) {
      // For create
      apiPath = "/api/organization-document/" + id;
      successMessage = "Document '" + this.state.title + "' updated."
      method = "PUT";
    } else {
...
console.log('thisstattetet--',JSON.stringify(this.state));
    const response = await fetch(apiPath, {
      ...
      .then((response) => {
        // console.log('responseresponseresponseresponse0- on OrganizationDocumentUpdates on OrganizationDocumentUpdates---', response, projectId);
        // Redirect to the Project component.
        var redirectTarget = "/paneldashboard/"
        var redirectIdOrgOrProject = orgId;
      ...
      .catch(err => {
        console.log('on OrganizationDocumentUpdates  error', err);
      });
  ...
  componentDidMount() {
console.log('dismount',this.props.location.state);
    ...
    // this.fetchPersons(orgId);
    if (parseInt(id) > 0) {
    fetch(`/api/organization-document-single/${id}`)
      .then(res => res.json())
      .then(response => {
        this.setState({
          id: id,
    ...
    if(orgId > 0){ //Need this codition in case of when creating KPI for organisation and ProjectId is not in exist - Issue #48
...
export default withStyles(styles)(OrganizationDocumentUpdate);
