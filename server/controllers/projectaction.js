...
module.exports = {
  ...
  findAll(req, res) {
    let sql =
      "select * from ProjectActions " +
      "where projid = " +
      req.params.projid +
      " and disabled = 0";
    logger.debug(
      `${callerType} Get ProjectActions by ProjectId -> sql: ${sql}`
    );
    return models.sequelize
      ...
      .then((_k) => {
        logger.debug(
          `${callerType} Get ProjectActions by ProjectId -> successful, count: ${_k.length}`
        );
        res.status(201).send(_k);
      })
      .catch((error) => {
        logger.error(
          `${callerType} Get ProjectActions by ProjectId -> error: ${error.stack}`
        );
  ...
  projectActionPersonsByProjectId(req, res) {
    ...
    return models.sequelize
      ...
      .then((p) => {
        logger.debug(
          `${callerType} projectActionPersonsByProjectId -> ProjectId: ${req.params.projectId}`
        );
        res.status(200).send(p);
      })
      .catch((error) => {
        logger.error(
          `${callerType} projectActionPersonsByProjectId -> error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
  ...
  async createProjectAction(req, res) {
    ...
    logger.debug(
      `${callerType} create -> New Project Action for project id : ${projectId}`
    );
      ...
      .then(async (ProjectAction) => {
        ...
        if (req.body.assigneeIds) {
          for (var i = 0; i < req.body.assigneeIds.length; i++) {
            ...
            try {
              await models.ProjectActionPerson.create({
                projectActionId,
                assigneeId,
              });
              var person = await models.Person.findByPk(assigneeId, {
                raw: true,
              });
              var to = person.email;
              ...
              var text = `<!doctype html>
                ...
                <table border="0" width="750" style="padding:0; margin:0;">
                  <tr>
                    <td style="font:17px Arial, Helvetica, sans-serif; color:#333;">Hello ${
                      person.firstName
                    }, </td>
                  </tr>
                  <tr>
                ...
                position: relative;
                top: -2px;
                margin-right: 10px;"></span> <em style="width:150px;font-style:normal;display:inline-block;"> Project Title </em> : ${
                  project.title
                }</td>
                  </tr>
                  <tr>
                ...
                position: relative;
                top: -2px;
                margin-right: 10px;"></span> <em style="width:150px;font-style:normal;display:inline-block;"> Assigned to </em> : ${person.firstName +
                  "" +
                  person.lastName}</td>
                  </tr>
                  <tr>
              ...
              logger.error(`error in send project action mail ::: ${e}`);
            }
          }
        }
  ...
  async updateProjectAction(req, res) {
    ...
    return models.ProjectAction.update(
      ...
      .then(async () => {
        ...
        await models.ProjectActionPerson.destroy({
          where: { projectActionId },
        });
        ...
        if (req.body.assigneeIds) {
          for (var i = 0; i < req.body.assigneeIds.length; i++) {
            ...
            try {
              ...
              const respons = await models.Person.findByPk(assigneeId, {
                raw: true,
              });
              ...
              var text =
                ...
                ", An existing Action '" +
                title +
                "' is updated recently that is assigned to you.";
              //mailer.sendMail(to,subject,text);
            } catch (e) {
              logger.error(`error in send project action mail ::: ${e}`);
            }
  ...
  async deactivateProjectAction(req, res) {
    const id = req.params.actionId;
    logger.debug(
      `${callerType} Deactivate -> Deactivate Project Action for id : ${id}`
    );
  ...
  orgnizationActionsCount(req, res) {
    ...
    return models.sequelize
      ...
      .then((result) => {
        logger.debug(
          `${callerType} orgnizationActionsCount successful, count: ${result.length}`
        );
        res.status(201).send(result);
      })
      .catch((error) => {
        logger.error(
          `${callerType} orgnizationActionsCount error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
