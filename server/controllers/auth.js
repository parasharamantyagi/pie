      ...
      .then((p) => {
        ...
        bCrypt.compare(req.body.password, p.pwdhash, function(err, result) {
          if (result === true) {
            ...
            let sql =
              "update Persons set lastLogin = CURRENT_TIMESTAMP where id = " +
              p.id;
    ...
    if (token !== undefined && token) {
      ...
      jwt.verify(token, config.get("security.jwtSecret"), function(
        ...
        if (err) {
          ...
          res.decoded = decoded;
          logger.debug(
            `${mvcType} validateToken -> decoded token is: ${decoded}`
          );
          _code = 200;
        }
      });
  ...
  reset(req, res) {
    ...
    return Person.findOne({
      ...
      .then((p) => {
        ...
        logger.debug(
          `${mvcType} create -> after hash, hash: ${hashedValue} pid ${p}`
        );
        ...
        models.Person.update(
          ...
          .then(([d, updatestatus]) => {
            ...
            if (updatestatus === 1) {
              ...
              text += "<br/><br/>";
              text +=
                "Your password has been reset successfully. Your account details are as follow:";
              text += "<br/><br/>";
              ...
              text += "<br/><br/>";
              text +=
                "To sign in to your account please visit - <a href='http://pie.value-infinity.com' target='_blank' >http://pie.value-infinity.com</a>";
              text += "<br/><br/>";
              text += "Teams";
              text += "<br/>";
              text += "Value-Infinity.";
  ...
  async saveAcl(req, res) {
    ...
    return models.sequelize
      ...
      .then((data) => {
        ...
        if (data.length > 0) {
          sql =
            "update Acls set jsonData = '" +
            jsonData +
            "',updatedAt=now() where orgId = " +
            orgId +
            ";";
        } else {
          sql =
            "insert into Acls (orgId, jsonData,createdAt,updatedAt) values (" +
            orgId +
            ",'" +
            jsonData +
            "',now(),now())";
