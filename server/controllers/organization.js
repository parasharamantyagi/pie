...
module.exports = {
  ...
  async fulltextSearch(req, res) {
    let searchTerm = req.headers.term ? req.headers.term : " ";
    let orgId = req.headers.orgid;
    logger.debug(
      `Organization fulltextSearch, params: ` + JSON.stringify(req.headers)
    );
  ...
  checkKpiPrioritizationLock(req, res) {
    ...
    return models.sequelize
      ...
      .catch((error) => {
        logger.error(
          `Organization checkKpiPrioritizationLock -> error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
  ...
  saveKpiPrioritizationLock(req, res) {
    ...
    logger.debug(
      `Organizations saveKpiPrioritizationLock --> name = ${req.params.name}`
    );
    return models.Organization.update(
      {
      ...
      .catch((error) => {
        logger.error(
          `Organizations saveKpiPrioritizationLock -> error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
  ...
  orgnizationProjectStatus(req, res) {
    ...
    return models.sequelize
      ...
      .then((result) => {
        logger.debug(
          `${callerType} orgnizationProjectStatus successful, count: ${result.length}`
        );
        res.status(201).send(result);
      })
      .catch((error) => {
        logger.error(
          `${callerType} orgnizationProjectStatus error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
  ...
  orgnizationProjectActionStatus(req, res) {
    ...
    return models.sequelize
      ...
      .then((result) => {
        logger.debug(
          `${callerType} orgnizationProjectActionStatus successful, count: ${result.length}`
        );
        res.status(201).send(result);
      })
      .catch((error) => {
        logger.error(
          `${callerType} orgnizationProjectActionStatus error: ${error.stack}`
        );
        res.status(400).send(error);
      });
  },
  async orgnizationProjectActioNewVsClose(req, res) {
    ...
    try {
      ...
      const closeResult = await models.sequelize.query(closeSql, {
        type: models.sequelize.QueryTypes.SELECT,
      });
      logger.debug(
        `${callerType} orgnizationProjectActioNewVsClose successful`
      );
      const newObject = {};
      const closeObject = {};
      const montes = {};
      ...
      res
        .status(201)
        .send({ newObject, closeObject, montes: Object.keys(montes) });
    } catch (error) {
      logger.error(
        `${callerType} orgnizationProjectActionStatus error: ${error.stack}`
      );
